...
local_dr_path <- file.path("../../local_dr_test")
...
3.  **Initialize the desk review**. By default, the start date of the desk review is 3 years from the end date on January 1st. The end date is 6 weeks from today. You may modify the default start and end dates.

```{r init}
ctry.data <- init_dr(country_name, 
                     local_dr_folder = local_dr_path, 
                     attach_spatial_data = attach_spatial_data,
                     branch = "59-dr-modularization-data-loading")

# Uncomment if you can access the folder directly from EDAV to get the lab data
# will also need lab.locs for the cleaning step
ctry.data$lab.data <- sirfunctions::edav_io("read", 
                                            file_loc = "Data/lab/2024-04-05 EMRO Lab Extract (AFP only).csv") |> filter(!is.na(EPID))
lab.locs <- lab.locs <- sirfunctions::edav_io("read", file_loc = "Data/lab/Routine_lab_testing_locations.csv")
...
1.  **Check for errors**. Run the following checks, if applicable. Error logs will be exported to the `errors` folder. Use `lab_data_errors_who` if the lab data came from WHO. Use `lab_data_errors_regional` if the lab data came from the regional office.

```{r check.errors}
ctry_data_errors(ctry.data)
lab_data_errors_who(ctry.data)
# lab_data_errors_regional function not yet complete
...
ctry.data$lab.data <- clean_lab_data_regional(ctry.data, region="EMRO", 
                                              lab.locs = lab.locs, 
                                              start_date, end_date, 
                                              delim = "/")
ctry.data$iss.data <- clean_iss_data(ctry.data$iss)
```
